...
def test_region_string_parser():
    ...
    # FASTA style names
    assert parse_region_string("gb|accession|locus") == (
        "gb|accession|locus",
        None,
        None,
    )
    assert parse_region_string("gb|accession|locus:1000-2000") == (
        "gb|accession|locus",
        1000,
        2000,
    )
    assert parse_region_string("gb|accession|locus:1,000-2,000") == (
        "gb|accession|locus",
        1000,
        2000,
    )
    ...
    assert parse_region_string("name-with-hyphens-") == (
        "name-with-hyphens-",
        None,
        None,
    )
...
def test_region_to_extent(mock_cooler):
    ...
    assert cooler.api.region_to_extent(mock_cooler, chromID_lookup, region, None) == (
        first,
        last + 1,
    )
